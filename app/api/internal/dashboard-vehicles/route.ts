...
let inFlightDashboardVehiclesRequest: Promise<any[]> | null = null;

async function fetchNormalizedDashboardVehicles() {
...
export async function GET(request: NextRequest) {
  ...
  try {
    if (!inFlightDashboardVehiclesRequest) {
      console.log('[INTERNAL API] Fetching from external API:', EXTERNAL_API);
      inFlightDashboardVehiclesRequest = fetchNormalizedDashboardVehicles();
    }

    const normalized = await inFlightDashboardVehiclesRequest;
    ...
    inFlightDashboardVehiclesRequest = null;
  }
}
